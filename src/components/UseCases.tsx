...
const UseCases = () => {
  ...
  return <section className="pt-12 md:pt-16 lg:pt-20 pb-8 md:pb-12 lg:pb-16 bg-dark-deeper relative">
      <GradientBackground />
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div role="region" aria-label="Use Cases Carousel" className="relative">
          ...
          <div className="hidden md:block absolute top-1/2 -left-4 transform -translate-y-1/2 z-10">
            <button onClick={scrollLeft} className="p-3 rounded-full bg-dark-deeper/80 backdrop-blur-sm text-white hover:bg-primary transition-colors" aria-label="Scroll left">
              <ArrowLeft size={24} />
            </button>
          </div>
          ...
          <div ref={scrollRef} role="list" className={cn("flex gap-6 pb-6", isMobile ? "flex-col" : "overflow-x-auto snap-x scroll-smooth hide-scrollbar")} onKeyDown={handleKeyNavigation} tabIndex={0}>
            {useCases.map((useCase, index) => <motion.div key={index} role="listitem" aria-label={`${useCase.title} - ${useCase.subtitle}`} className={cn("bg-dark-deeper/50 backdrop-blur-sm border border-white/5 rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 focus:ring-2 focus:ring-primary/50", isMobile ? "w-full" : "min-w-[300px] flex-shrink-0 snap-start")} variants={cardVariants} initial="initial" whileHover="hover">
                <div className="relative">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="px-3 py-1.5 text-xs font-medium rounded-full bg-white/5 backdrop-blur-sm border border-white/10 text-white/90 hover:bg-white/10 transition-colors">
                      {useCase.category}
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  ...
                  {/* Stats */}
                  <div className="text-sm text-primary/90 font-medium mb-4">
                    {useCase.stats}
                  </div>
          ...
          <div className="hidden md:block absolute top-1/2 -right-4 transform -translate-y-1/2 z-10">
            <button onClick={scrollRight} className="p-3 rounded-full bg-dark-deeper/80 backdrop-blur-sm text-white hover:bg-primary transition-colors" aria-label="Scroll right">
              <ArrowRight size={24} />
            </button>
          </div>
          ...
          <div className="hidden md:flex justify-center gap-2 mt-6">
            {useCases.map((_, index) => <button key={index} onClick={() => scrollToCard(index)} className={cn("w-2 h-2 rounded-full transition-colors", activeIndex === index ? "bg-primary" : "bg-white/30")} aria-label={`Go to slide ${index + 1}`} />)}
          </div>
        </div>
      </div>
    </section>;
