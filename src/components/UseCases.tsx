...
const UseCases = () => {
  ...
  return (
    <section className="pt-12 md:pt-16 lg:pt-20 pb-8 md:pb-12 lg:pb-16 bg-dark-deeper relative">
      <GradientBackground />
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        >
          ...
          <div className="hidden md:block absolute top-1/2 -left-4 transform -translate-y-1/2 z-10">
            <button 
              onClick={scrollLeft} 
              className="p-3 rounded-full bg-dark-deeper/80 backdrop-blur-sm text-white hover:bg-primary transition-colors"
          ...
          <div 
            ...
            className={cn(
              "flex gap-6 pb-6",
              isMobile ? "flex-col" : "overflow-x-auto snap-x scroll-smooth hide-scrollbar"
            )}
          ...
          >
            {useCases.map((useCase, index) => (
              <motion.div 
                key={index} 
                role="listitem"
                aria-label={`${useCase.title} - ${useCase.subtitle}`}
                className={cn(
                  "bg-dark-deeper/50 backdrop-blur-sm border border-white/5 rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 focus:ring-2 focus:ring-primary/50",
                  isMobile ? "w-full" : "min-w-[300px] flex-shrink-0 snap-start"
                )}
