...
const CommandDialog = ({ children, ...props }: DialogProps) => {
  return (
    <Dialog {...props}>
      ...
      <DialogContent className="overflow-hidden p-0 shadow-lg sm:max-w-lg [&>button:last-child]:hidden">
...
>(({ className, ...props }, ref) => (
  <div className="flex items-center border-b border-border px-5" cmdk-input-wrapper="">
    <MagnifyingGlassIcon className="me-3 h-4 w-4 text-muted-foreground" />
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        "flex h-10 w-full bg-transparent py-3 text-sm outline-none ring-0 focus:outline-none focus:ring-0 placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50",
        className,
      )}
      {...props}
    />
