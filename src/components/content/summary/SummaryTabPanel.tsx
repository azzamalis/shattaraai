...
export function SummaryTabPanel({
  ...
  return <div className="h-[calc(100vh-165px)] overflow-y-auto md:h-[calc(100vh-120px)]">
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center">
                  <Button id="summary-generate-btn" onClick={handleCreateSummary} disabled={isLoading} className="rounded-l-full rounded-r-none border-r border-background/20 bg-foreground text-background hover:bg-foreground/90 px-4 h-10">
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button disabled={isLoading} className="h-10 rounded-l-none rounded-r-full bg-foreground text-background hover:bg-foreground/90 px-2">
          ...
          <div className="bg-transparent rounded-2xl border-2 border-border p-6 shadow-sm px-4 py-4">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <h3 className="text-base text-foreground mb-1 font-medium">
          ...
          {summaries.length > 0 && <div className="space-y-3 pt-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-medium text-muted-foreground">My Summaries</h3>
              </div>
              <div className="flex flex-col gap-3">
                {summaries.map(summary => <div key={summary.id} onClick={() => onViewSummary(summary)} className="group cursor-pointer bg-transparent rounded-2xl border-2 border-border p-4 shadow-sm transition-all duration-200 hover:border-foreground/30 hover:bg-accent/30">
                    <div className="flex items-start justify-between gap-6">
                      <div className="min-w-0 flex-1 space-y-2">
                        <div className="flex min-w-0 items-center gap-2 text-base font-medium text-foreground">
                          <span className="truncate text-base font-medium">{summary.title}</span>
                        </div>
                        <div className="flex flex-wrap items-center gap-1">
                          <div className={cn('truncate rounded-full px-2 py-1 text-xs', summaryTypeBadgeStyles[summary.type])}>
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="icon" className="h-5 w-5 flex-shrink-0 p-0 opacity-0 transition-opacity duration-200 group-hover:opacity-100" onClick={e => e.stopPropagation()}>
