...
export const FlashcardListDisplay = ({
  ...
  return <div className="group cursor-pointer bg-transparent rounded-2xl border-2 border-border p-4 shadow-sm transition-all duration-200 hover:border-foreground/30 hover:bg-accent/30" onClick={isEditing ? undefined : onStartFlashcards}>
      <div className="flex items-start justify-between gap-6">
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div className="flex min-w-0 items-center gap-2 text-base font-medium text-foreground">
            ...
            {isEditing ? <Input ref={inputRef} value={editTitle} onChange={e => setEditTitle(e.target.value)} onKeyDown={handleKeyDown} onBlur={handleCancelRename} className="text-foreground font-medium h-7 w-full max-w-[200px] text-sm" onClick={e => e.stopPropagation()} /> : <span className="truncate text-base font-medium">{title}</span>}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-1">
            ...
            {concepts.length > 0 && <>
                ...
                {displayConcepts.map((concept, index) => <span key={index} className="text-[#00A3FF] bg-[#00A3FF]/10 py-0.5 px-2 rounded-full text-xs">
                    {concept}
                  </span>)}
                {!showAllConcepts && remainingConcepts > 0 && <button onClick={e => {
              e.stopPropagation();
          ...
          <div className="flex items-center gap-3 pt-1">
            <div className="bg-muted flex-grow rounded-full h-2">
              <div className={`${getProgressColor()} rounded-full h-full transition-all`} style={{
              width: `${progress}%`
            }} />
            </div>
