...
export function LearningCardTitle({
  ...
  if (isEditing && !disabled) {
    return <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
        ...
        <Button size="sm" variant="ghost" onClick={handleSave} disabled={isSaving} className="h-5 w-5 p-0 hover:bg-green-100">
          <Check className="h-3 w-3 text-green-600" />
        </Button>
        <Button size="sm" variant="ghost" onClick={handleCancel} disabled={isSaving} className="h-5 w-5 p-0 hover:bg-red-100">
