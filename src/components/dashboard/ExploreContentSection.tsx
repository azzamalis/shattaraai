...
export function ExploreContentSection({
  ...
  return <section className="w-full py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <button className={cn("text-sm text-muted-foreground", "hover:text-foreground transition-colors", "px-4 py-2 rounded-lg", "hover:bg-accent/50")}>
          View all
        </button>
      </div>
      ...
      <div className="relative -mx-4 px-4">
        ...
        dragFree: true
          ...
          <CarouselContent className="-ml-4">
            {exploreContentData.map(item => <CarouselItem key={item.id} className={cn("pl-4", "basis-[280px] sm:basis-[320px] md:basis-[360px]", "first:pl-4")}>
                <LearningCard content={item} onDelete={() => onDeleteCard(item)} onShare={() => onShareCard(item)} />
              </CarouselItem>)}
          </CarouselContent>
