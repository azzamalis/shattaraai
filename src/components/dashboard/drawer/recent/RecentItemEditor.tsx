...
export const RecentItemEditor: React.FC<RecentItemEditorProps> = ({
  ...
  return <div className="flex items-center gap-2 flex-1">
      ...
      <Button size="sm" onClick={onSave} variant="ghost" className="h-8 w-8 p-0 text-green-500">
        <Check className="h-4 w-4" />
      </Button>
      <Button size="sm" variant="ghost" onClick={onCancel} className="h-8 w-8 p-0 text-red-500">
        <X className="h-4 w-4" />
      </Button>
    </div>;
};