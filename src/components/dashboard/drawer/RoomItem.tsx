...
export const RoomItem: React.FC<RoomItemProps> = ({
  ...
  return <div className="flex items-center justify-between gap-2">
      {editingRoomId === room.id ? <div className="flex items-center gap-2 flex-1">
          ...
          <Button size="sm" onClick={e => onSaveRename(e, room.id)} variant="ghost" className="h-8 w-8 p-0 text-green-500">
            <Check className="h-4 w-4" />
          </Button>
          <Button size="sm" variant="ghost" onClick={onCancelRename} className="h-8 w-8 p-0 text-red-500">
            <X className="h-4 w-4" />
          </Button>
        </div> : <>
