...
export function RoomContentTable({
  ...
  return <div className="w-full flex flex-col pt-8 py-[20px]">
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b border-dashboard-separator">
            {showSelectionColumn && <th className="text-left py-4 px-4 font-medium text-dashboard-text text-base w-10">
                Select
              </th>}
          ...
          return <tr key={item.id} className="border-b border-dashboard-separator hover:bg-dashboard-card-hover transition-colors duration-200">
                ...
                <td className="py-6 px-4">
                  <div className="flex flex-wrap gap-2 justify-center">
                    {contentTags.map(tag => <span key={tag} className={`px-3 py-1 rounded-full text-xs font-medium ${getTagColor(tag as ContentTag)}`}>
                        {tag}
                      </span>)}
                  </div>
                </td>
      ...
      <div className="flex items-center justify-between mt-4 mb-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} className="h-8 w-8 p-0 hover:bg-accent hover:text-primary">
            <ChevronLeft className="h-4 w-4" />
          </Button>
          {Array.from({
          length: totalPages
        }, (_, i) => i + 1).map(page => <Button key={page} variant={currentPage === page ? "default" : "outline"} size="sm" onClick={() => setCurrentPage(page)} className={`h-8 w-8 p-0 ${currentPage === page ? "bg-primary text-primary-foreground hover:bg-primary/90" : "hover:bg-accent hover:text-primary"}`}>
              {page}
            </Button>)}
          <Button variant="outline" size="sm" onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} className="h-8 w-8 p-0 hover:bg-accent hover:text-primary">
      ...
      {/* Delete Modal */}
      <DeleteModal open={deleteModalOpen} onOpenChange={handleDeleteModalClose} type="content" itemToDelete={{
      id: selectedItem?.id || '',
      title: selectedItem?.title || ''
