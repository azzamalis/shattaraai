        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col md:flex-row h-full min-h-0">
          ...
          <div className="block md:hidden border-b border-border p-2">
            <div className="flex justify-between items-center mb-3">
              <Button variant="ghost" size="sm" onClick={() => onOpenChange(false)} className="rounded-lg h-8 w-8 p-0">
                <X className="h-4 w-4" />
              </Button>
            </div>
            <TabsList className="w-full h-auto p-0 bg-transparent flex gap-1 overflow-x-auto touch-pan-x">
              {tabs.map(tab => <TabsTrigger key={tab.id} value={tab.id} className="flex-shrink-0 flex items-center gap-2 px-3 py-2 text-sm rounded-lg transition-colors whitespace-nowrap font-medium touch-manipulation relative data-[state=active]:bg-muted data-[state=active]:text-foreground data-[state=inactive]:text-primary/70 data-[state=inactive]:hover:text-foreground data-[state=inactive]:hover:bg-muted/50">
          ...
          <div className="hidden md:block w-48 bg-muted/50 border-r border-border p-2">
            ...
            <TabsList className="w-full h-auto p-0 bg-transparent flex flex-col space-y-1">
              {tabs.map(tab => <TabsTrigger key={tab.id} value={tab.id} className="w-full flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors relative justify-start whitespace-nowrap data-[state=active]:bg-muted data-[state=active]:text-foreground data-[state=inactive]:text-primary/70 data-[state=inactive]:hover:text-foreground data-[state=inactive]:hover:bg-muted/50">
