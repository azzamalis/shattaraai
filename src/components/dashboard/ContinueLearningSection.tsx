  ...
  if (content.length === 0) {
    return <div className="mt-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-neutral-950 text-xl font-semibold">Continue learning</h2>
        </div>
      ...
      <div className="relative">
        ...
        loop: false
          ...
          <CarouselContent className="-ml-2 md:-ml-4">
            {content.map(item => <CarouselItem key={item.id} className="pl-2 md:pl-4 basis-auto">
                <LearningCard content={item} onDelete={() => onDeleteCard(item.id)} onShare={onShareCard} />
              </CarouselItem>)}
          </CarouselContent>
          {content.length > 1 && <>
