...
export function ChatTabNavigation({
  ...
  return <div>
      <Tabs value={activeTab} onValueChange={value => onTabChange(value as ChatTabType)} className="w-full bg-background ">
        <TabsList className={cn("w-[calc(100%-2rem)] justify-start gap-1 p-1 h-12 shrink-0 mx-4 mt-2 mb-4", "bg-dashboard-card-hover dark:bg-dashboard-card-hover", "transition-colors duration-200", "rounded-xl")}>
          {tabs.map(tab => {
          const Icon = tab.icon;
